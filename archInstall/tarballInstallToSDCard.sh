...
while [ -z ${newDisk+x} ]; do
  ...
  if [[ $newDisks == "" ]]; then
    ...
    while read; do
          ...
          if egrep -q "Generic.*SD" <<<"$diskInfo"; then
...
diskutil partitionDisk $newDisk MBR fat32 BOOT 300M fat32 REFORMAT R >/dev/null 2>/dev/null
...
tar -zxf "$TARBALL" -C RAMFS \
  ./usr/bin/{fdisk,ln,ls,cp,mkfs.ext4,reboot,systemctl,bash,sed,tar,bsdtar,{u,}mount,echo,mkdir,chmod,setsid,sync,sleep} \
...
bsdtar -zxf /mnt/ArchLinuxARM-rpi-2-latest.tar.gz -C newroot 
